 ...
 *
 *   macOS:   …/Benaiah.app/Contents/MacOS/Benaiah → …/Benaiah.app
 *   Windows: …\Benaiah\Benaiah.exe                → …\Benaiah (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Benaiah.app/Contents/MacOS/Benaiah → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Benaiah.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Benaiah.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/](?:Benaiah|Hermes)$/i.test(dir) || /[\\/](?:benaiah|hermes)-desktop$/i.test(dir)) {
      return dir
    }
