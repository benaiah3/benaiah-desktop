 ...
 *
 * Stamps the Benaiah icon + identity onto the packed Windows Benaiah.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Benaiah')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Benaiah'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Benaiah.exe keeps the stock Electron icon`)
  }
}
