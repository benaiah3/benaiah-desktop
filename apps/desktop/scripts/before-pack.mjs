 ...
 * product name (`Benaiah`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Benaiah'
 *
 ...
 * (the mac-unpacked Benaiah.app bundle) and Windows (win-unpacked), so we
 ...
 * Benaiah.exe that Windows can't load (truncated PE from a corrupt cached
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Benaiah.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Benaiah'}.exe`
