#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Benaiah icon + version metadata onto the
// built Benaiah.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Benaiah.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Benaiah.exe>
//
