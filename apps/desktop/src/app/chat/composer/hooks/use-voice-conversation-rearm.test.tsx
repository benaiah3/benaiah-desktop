...
const mocks = vi.hoisted(() => {
  ...
  const stopVoicePlayback = vi.fn((options: { userInitiated?: boolean } = {}) => {
  ...
  const consumeUserStopRequested = vi.fn(() => {
...
vi.mock('@/lib/voice-playback', () => ({
  consumeUserStopRequested: mocks.consumeUserStopRequested,
...
describe('useVoiceConversation playback rearm', () => {
  ...
  it('honors Stop while streaming playback is still preparing', async () => {
    ...
    mocks.stopVoicePlayback({ userInitiated: true })
    await act(async () => {
