...
describe('DesktopInstallOverlay first-run setup', () => {
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    expect(screen.queryByText('Local installation could not start. Restart Benaiah Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    expect(screen.queryByText('Local installation could not start. Restart Benaiah Desktop and try again.')).toBeNull()
  })
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    expect(await screen.findByText('Benaiah needs a one-time install')).toBeTruthy()
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('Benaiah needs a one-time install')).toBeNull()
  })
})
