...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Benaiah gateway connection closed',
      connectErrorMessage: 'Could not connect to Benaiah gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Benaiah gateway is not connected',
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.hermesDesktop?.api) {
    throw new Error('Benaiah desktop bridge unavailable')
  }
