...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    # Host CLIs (e.g. Homebrew whisper) must not import the Hermes venv's
    # site-packages via a leaked PYTHONPATH — that path caused NumPy ABI
    # hard-crashes that took down the managed engine.
    for _py_marker in ("PYTHONPATH", "PYTHONHOME", "__PYVENV_LAUNCHER__"):
        env.pop(_py_marker, None)
