...
def _get_provider(stt_config: dict) -> str:
    ...
    if explicit:
        if provider == "local":
            if _HAS_FASTER_WHISPER:
                return "local"
            # Prefer lazy in-venv faster-whisper over host CLI (Homebrew whisper
            # often inherits a broken NumPy/OpenMP stack and can hard-crash).
    ...
    if _HAS_FASTER_WHISPER:
        return "local"
    # Prefer lazy in-venv faster-whisper before any host whisper CLI.
    if _try_lazy_install_stt():
        return "local"
    if _has_local_command():
        return "local_command"
